...
Span* PageCache::MapAddrToSpan(void *addr)
{
    ...
    if(it == PageIdToSpan.end()) assert(false);
    return it->second;
}
...
void PageCache::ReleaseSpanToPageCache(Span *span)
{
    ...
    while(true)
    {
        ...
        if(it == PageIdToSpan.end() || it->second->isusing || span->n + it->second->n > 128) break;
    ...
    while(true)
    {
        ...
        //如果找不到或者找到的页正在被使用或者找到的页大小合并后总大小大于NPAGES,则退出循环
        if(it == PageIdToSpan.end() || it->second->isusing || span->n + it->second->n > NPAGES) break;
